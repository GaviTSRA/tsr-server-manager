import { Play, X, Square } from "react-feather";
import { trpc } from "../main";
import { MoonLoader } from "react-spinners";

export function ServerControls({ serverId, nodeId, status }: {
  serverId: string,
  nodeId: string,
  status: "created" | "running" | "paused" | "restarting" | "removing" | "exited" | "dead" | undefined,
}) {
  ...
  const genericClassName = "cursor-pointer bg-neutral-400 rounded-lg px-2 py-1 w-full h-full flex flex-row gap-2 items-center text-center";

  let controls: JSX.Element = <p>Unhandled</p>
  if (startServer.isPending || stopServer.isPending || killServer.isPending) {
    controls = <div className={genericClassName + " justify-center my-1"}>
      <MoonLoader size={16} color={"#fff"} />
    </div>
  } else if (status === undefined || status === "created" || status === "exited") {
    controls = <div
      onClick={() =>
