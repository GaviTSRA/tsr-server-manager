...
serverRouter.get("/", async (req: ServerRouterRequest, res) => {
    const result = req.body.server.containerId ? await docker.inspectContainer(req.body.server.containerId) : undefined;
...
serverRouter.post("/start", async (req: ServerRouterRequest, res) => {
    if (req.body.server.state === "NOT_INSTALLED") {
        ...
        const type = serverTypes.find(type => type.id === req.body.server.type) as ServerType;
        ...
        if (!containerId) {
            ...
            await db.update(schema.Server).set({ containerId: result.containerId }).where(eq(schema.Server.id, req.body.server.id))
            containerId = result.containerId;
        }

        res.write("[TSR Server Manager] Installing server...\n");
        res.write("[TSR Server Manager] Creating install script...\n");
        fs.copyFileSync(`servertypes/${type.id}/install.sh`, `servers/${id}/install.sh`);
        ...
        res2.data.on("end", async () => {
            ...
            await db.update(schema.Server).set({ state: "INSTALLED" }).where(eq(schema.Server.id, id));
        })
        return;
    }
...
serverRouter.post("/options", async (req: ServerRouterRequest, res) => {
    ...
    await db.update(schema.Server).set({ options: req.body.options }).where(eq(schema.Server.id, req.body.server.id));
    res.sendStatus(200);
})
...
serverRouter.get("/connect", async (req, res) => {
    ...
    try {
        const logsResponse = await axios.get(`http://localhost:2375/containers/${req.body.server.containerId}/logs?stdout=true&stderr=true&logs=true`);
...
fs.readdirSync("servertypes").forEach((folder) => {
    const json = fs.readFileSync(`servertypes/${folder}/manifest.json`).toString();
...
serverTypes.forEach((type) => {
    if (type.image && !requiredImages.includes(type.image)) requiredImages.push(type.image);
