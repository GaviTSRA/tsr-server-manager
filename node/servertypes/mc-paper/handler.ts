...
export async function handleEvent(event: PlatformEvent) {
  switch (event.type) {
    ...
    case "log":
      const serverOnline = /\[(\d\d:\d\d:\d\d) INFO\]: Done \(([0-9.]+)s\)!/;
      const serverOffline = /\[(\d\d:\d\d:\d\d) INFO\]: Closing Server/;
      const join = /\[(\d\d:\d\d:\d\d) INFO]: \[33m\[93m(\w+) joined the game/;
      const leave = /\[(\d\d:\d\d:\d\d) INFO]: \[33m\[93m(\w+) left the game/;
